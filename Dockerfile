...
RUN rm /opt/selenium/selenium-server-standalone.jar \
  && wget --no-verbose -O /opt/selenium/selenium-server-standalone.jar https://testimstatic.blob.core.windows.net/staging/selenium-server-standalone-$GRID_JAR_VERSION.jar
...
RUN apt-get update -qqy \
  ...
  && echo $(curl -sI -X HEAD "${FIREFOX_PROFILE_URL}" | tr -d '\r' | sed -En 's/^ETag: (.*)/\1/p') > /opt/testim-headless-info \
  && rm -rf /var/lib/apt/lists/* \
